...
public:
    ...
    uint32_t root_addr; // root node address;
    uint32_t capacity;  // max size;
    uint32_t allocated; // allocated size;
    uint32_t node_start;// start/offset of node;
    uint32_t data[0];   // nodes, append only.

    static node_manager *load_file(string name);
    int unload_file();
    node_manager() {}
    node_manager(string name, bool for_index, int root_addr, int capacity);
    // align with page size.
    size_t get_node_start() { return PAGE_SIZE; }
    int append_page(string file_name, char *page, size_t size);
};

node_manager::node_manager(string name, bool for_index, int root_addr, int capacity)
{
    this->head = (name[0]<<24)+(name[1]<<16)+(name[2]<<8)+name[3];
    this->type = for_index ? 1 : 2;
    this->root_addr = root_addr;
    this->capacity  = capacity;
    this->node_start  = get_node_start();
...
int
node_manager::append_page(string file_name, char *page, size_t size)
{
    int fd = open(file_name.c_str(), O_APPEND);
    if (fd == -1) return -1;
    int sz = write(fd, page, size);
    cout << sz << "(" << size << ") bytes written." << endl;
    int re = close(fd);
    if (re == -1) return -2;
    return sz;
}
...
int map_file_begin(map_handle *h)
{
    h->fd = open(h->file_path, O_RDWR);
    if (h->fd == -1) return -1;
    h->length = lseek(h->fd, 0, SEEK_END);
    h->buffer = (char *)mmap(NULL, h->length,
